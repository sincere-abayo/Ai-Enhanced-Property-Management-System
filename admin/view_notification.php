...
if ($notification['type'] === 'payment') {
    ...
    $stmt = $pdo->prepare("
        SELECT p.*, l.property_id, pr.property_name, u.first_name, u.last_name
        FROM payments p
        JOIN leases l ON p.lease_id = l.lease_id
        JOIN properties pr ON l.property_id = pr.property_id
        JOIN users u ON l.tenant_id = u.user_id
        WHERE p.payment_id = :paymentId AND pr.landlord_id = :userId
        LIMIT 1
    ");
    ...
    $stmt = $pdo->prepare("
        SELECT m.*, pr.property_name, u.first_name, u.last_name
        FROM maintenance_requests m
        JOIN properties pr ON m.property_id = pr.property_id
        JOIN users u ON m.tenant_id = u.user_id
        WHERE m.request_id = :requestId AND pr.landlord_id = :userId
        LIMIT 1
    ");
    ...
    $stmt = $pdo->prepare("
        SELECT l.*, pr.property_name, u.first_name, u.last_name
        FROM leases l
        JOIN properties pr ON l.property_id = pr.property_id
        JOIN users u ON l.tenant_id = u.user_id
        WHERE l.lease_id = :leaseId AND pr.landlord_id = :userId
        LIMIT 1
    ");
